...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key === 'ArrowUp' || event.key === 'ArrowDown' ||
            ...
            if (event.key === 'ArrowUp' && y <= 0.9) y += 0.01;
            else if (event.key === 'ArrowDown' && y >= -0.9) y -= 0.01;
            else if (event.key === 'ArrowLeft' && x >= -0.9) x -= 0.01;
            else if (event.key === 'ArrowRight' && x <= 0.9) x += 0.01;
        }
    });
